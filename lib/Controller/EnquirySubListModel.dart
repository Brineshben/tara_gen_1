...
class EnquirySubListController extends GetxController {
  ...
  Future<void> fetchEnquirySubList(int userID,int enquiry) async {
    ...
    try {
      List<dynamic> resp = await ApiServices.EnquiryListSub(userId: userID, enquiry: enquiry);

      if (resp.isNotEmpty) {
        enquiryList.value = resp.map((e) =>EnquirySubListModel.fromJson(e)).toList();
        // enquiryData.value = enquiryList.value?.enquiryDetails ?? [];
        print("Error fetching enqui${ enquiryList.value}");
        isLoaded.value = true;
      } else {
        isError.value = true;
      }
