...
class CustomerdetailsController extends GetxController {
  ...
  Future<void> customerData(
      {required String sessionId,required String username,required String purpose}) async {
    isLoading.value = true;
    isLoaded.value = false;
    try {
      Map<String, dynamic> resp =
      await ApiServices.customerDetails(sessionId: sessionId, userName: username, purPose: purpose);
      print("------respbwnenen------$resp");
      if (resp['status'] == "ok") {
        CustomerDetailsModel customerDetailsApi = CustomerDetailsModel.fromJson(resp);
